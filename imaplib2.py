...
class IMAP4(object):
    ...
    Instantiate with:
        ...
        identifier - string to be used in thread name (makes debug
                     output easier to read when multiple mailboxes are
                     open).
            ...
            try:
                ...
                if state & select.POLLIN:
                    ...
                    if dlen == 0:
                        rxzero_count += 1
                        if rxzero_count > 5:
                            raise Exception("connection appears to be broken")
                        time.sleep(0.1)
                    else:
                        rxzero_count = 0
                    while True:
...
class IMAP4_SSL(IMAP4):
    ...
    def __init__(self, host=None, port=None, keyfile=None, certfile=None, debug=None, debug_file=None, identifier=None, ssl_version=None):
    ...
    def open(self, host=None, port=None):
        ...
        try:
                import ssl
                if self.ssl_version:
                    self.sslobj = ssl.wrap_socket(self.sock, self.keyfile, self.certfile, ssl_version=self.ssl_version)
                else:
